  ...
  Widget _buildGraphFromPayload(
    ...
    if (graphType == 'emotion_trend_bar' && source == 'emotion_logs') {
      ...
      for (int i = 6; i >= 0; i--) {
        ...
        final logsForDay = appProvider.getEmotionLogsForDate(dateString);
        if (logsForDay.isNotEmpty) {
          ...
          dailyAverageEmotions.add(MapEntry(dateString, 0.0));
        }
      }

      if (dailyAverageEmotions.where((e) => e.value > 0).isEmpty) {
        return Text("Not enough emotion data for the past 7 days to display a trend.",
          ...
          BarChartData(
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (double value, TitleMeta meta) {
                    final date =
                        DateTime.parse(dailyAverageEmotions[value.toInt()].key);
              ...
              leftTitles: AxisTitles(
                  sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (v, m) => Text(v.toInt().toString(),
                          style: TextStyle(
                              fontSize: 10, color: AppTheme.fhTextSecondary)))),
              topTitles:
            ...
            barGroups: dailyAverageEmotions.asMap().entries.map((entry) {
              ...
              return BarChartGroupData(
                ...
                barRods: [
                  BarChartRodData(
                    toY: avgRating > 0 ? avgRating : 0.1,
                    color: avgRating > 0
                        ? dynamicAccent.withOpacity(0.8)
                        : AppTheme.fhTextDisabled.withOpacity(0.3),
                    width: 16,
